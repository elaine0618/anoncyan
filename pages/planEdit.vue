...
import { saveImage, deleteImage, cleanUnusedImages } from '@/utils/imageManager.js';
...
export default {
  ...
  methods: {
    ...
    async deleteAction(section, index) {
      const action = this.plan[section][index];
    ...
    chooseImage() {
      uni.chooseImage({
        ...
        success: async (res) => {
          uni.showLoading({ title: '添加图片...' });
          try {
            const savedPath = await saveImage(res.tempFilePaths[0]);
            this.currentAction.image = savedPath;
            uni.hideLoading();
            uni.showToast({ title: '图片已添加', icon: 'success' });
          } catch (err) {
            uni.hideLoading();
            uni.showToast({ title: '添加失败请检查权限', icon: 'none' });
          }
        }
      });
    },
    ...
    confirmEdit() {
      ...
      if (this.currentIndex === -1) {
        ...
        const oldAction = this.plan[this.currentSection][this.currentIndex];
        if (oldAction.image && oldAction.image !== actionData.image) {
          deleteImage(oldAction.image);
        }
    ...
    collectUsedImages() {
      ...
      sections.forEach(section => {
        if (this.plan[section]) {
          this.plan[section].forEach(action => {
            if (action.image) {
              used.push(action.image);
            }
          });
        }
      });
      this.usedImages = used;
    },
    ...
    async savePlan() {
      ...
      this.collectUsedImages();
      await cleanUnusedImages(this.usedImages);
